...
public class AppConfig {
	...
	public static boolean isTopActivity(Context activity,String shortActivityName){  
        String packageName = "org.tang.exam."+shortActivityName;  
        ActivityManager activityManager = (ActivityManager) activity.getSystemService(Context.ACTIVITY_SERVICE);  
        List<RunningTaskInfo>  tasksInfo = activityManager.getRunningTasks(1);    
        if(tasksInfo.size() > 0){    
            System.out.println("---------------包名-----------"+tasksInfo.get(0).topActivity.getShortClassName());  
            //应用程序位于堆栈的顶层    
            if(packageName.equals(tasksInfo.get(0).topActivity.getPackageName()+"."+shortActivityName)){    
                return true;    
            }    
        }    
        return false;  
