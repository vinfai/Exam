...
public class IndexScroller {
	...
	public void draw(Canvas canvas) {
		...
		indexbarPaint.setAlpha((int) (96 * mAlphaRate));
		...
		if (mSections != null && mSections.length > 0) {
			...
			if (mCurrentSection >= 0) {
				...
				canvas.drawText(mSections[mCurrentSection], previewRect.left
						+ (previewSize - previewTextWidth) / 2 - 1, previewRect.top + mPreviewPadding
			...
			for (int i = 0; i < mSections.length; i++) {
				...
				canvas.drawText(mSections[i], mIndexbarRect.left + paddingLeft, mIndexbarRect.top
						+ mIndexbarMargin + sectionHeight * i + paddingTop - indexPaint.ascent(), indexPaint);
			}
		}
	}
	...
	public void onSizeChanged(int w, int h, int oldw, int oldh) {
		mListViewWidth = w;
		mListViewHeight = h;
		mIndexbarRect = new RectF(w - mIndexbarMargin - mIndexbarWidth, mIndexbarMargin, w - mIndexbarMargin,
				h - mIndexbarMargin);
	}
